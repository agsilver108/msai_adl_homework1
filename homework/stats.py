...
def memory_profile(device):
    if device == "cuda":
        ...
        mem = MemoryProfile()
        # Suppress the profiler warning about clearing events
        with warnings.catch_warnings():
            warnings.filterwarnings("ignore", message=".*Profiler clears events.*")
            with profile(activities=[torch.profiler.ProfilerActivity.CPU], profile_memory=True, with_stack=True) as prof:
                yield mem
